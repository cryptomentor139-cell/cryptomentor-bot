            ...
            if had_open_position and not open_positions:
                ...
                try:
                    ...
                    for db_trade in open_db_trades:
                        ...
                        # StackMentor unified mode closes full size at TP.
                        # Preserve TP-hit flags for dashboard/performance views.
                        try:
                            if close_status == "closed_tp" and str(db_trade.get("strategy") or "").lower() == "stackmentor":
                                from app.supabase_repo import _client as _sc
                                _sc().table("autotrade_trades").update({
                                    "tp1_hit": True,
                                    "tp2_hit": True,
                                    "tp3_hit": True,
                                ...
                                }).eq("id", db_trade["id"]).execute()
                        except Exception as _tpflag_err:
                            logger.debug(f"[Engine:{user_id}] TP flag update skipped: {_tpflag_err}")
