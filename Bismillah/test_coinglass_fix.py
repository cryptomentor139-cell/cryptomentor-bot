...
try:
    ...
    print("❌ Failed to import CoinGlassProvider")
    exit(1)
...
def test_coinglass_connection():
    ...
    if 'error' not in comprehensive:
        ...
        print(f"❌ Comprehensive: {compcomprehensive = provider.get_comprehensive_futures_data(symbol)
        if 'error' not in comprehensive:
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("💡 Check API key and endpoint configurations")💡 Check API key validity or contact CoinGlass support")
