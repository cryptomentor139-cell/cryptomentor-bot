...
Try again in a few minutes for real-time data."""
    ...
    def analyze_snd_for_auto_signals(self, symbol, crypto_api):
        """Analyze Supply & Demand for auto signal generation with enhanced SnD logic"""
        try:
            # Check deployment mode
            is_deployment = crypto_api.is_deployment_mode() if hasattr(crypto_api, 'is_deployment_mode') else False
            
            # Get current price data with CoinAPI
            price_data = crypto_api.get_coinapi_price(symbol, force_refresh=is_deployment)
            ...
            # Get Binance candlestick data for SnD analysis
            try:
                candle_data = crypto_api.get_binance_candlestick(symbol, '1h', 50)
                if 'error' not in candle_data:
                    candlesticks = candle_data.get('candlesticks', [])
                else:
                    candlesticks = []
            except Exception as e:
                print(f"⚠️ Candlestick data error for {symbol}: {e}")
                candlesticks = []
            
            # Perform SnD analysis
            snd_result = self._perform_snd_analysis(symbol, current_price, change_24h, volume_24h, candlesticks)
    ...
    def _perform_snd_analysis(self, symbol, current_price, change_24h, volume_24h, candlesticks):
        """Perform Supply and Demand analysis using price action"""
        try:
            # Extract price levels from candlesticks
            highs = [c['high'] for c in candlesticks[-20:]] if candlesticks else []
            lows = [c['low'] for c in candlesticks[-20:]] if candlesticks else []
            closes = [c['close'] for c in candlesticks[-20:]] if candlesticks else []
            volumes = [c['volume'] for c in candlesticks[-20:]] if candlesticks else []
            
            # Find supply (resistance) and demand (support) zones
            supply_zones = self._identify_supply_zones(highs, volumes, current_price)
            demand_zones = self._identify_demand_zones(lows, volumes, current_price)
            
            # Determine signal based on SnD analysis
            signal_data = self._determine_snd_signal(current_price, change_24h, volume_24h, supply_zones, demand_zones)
    ...
    def _identify_supply_zones(self, highs, volumes, current_price):
        """Identify supply/resistance zones"""
        supply_zones = []
        ...
        for i in range(2, len(highs) - 2):
            ...
            if (highs[i] > highs[i-1] and highs[i] > highs[i-2] and 
                ...
                volume_strength = volumes[i] if i < len(volumes) else 1
                price_level = highs[i]
        ...
        supply_zones.sort(key=lambda x: x['distance'])
        return supply_zones[:3]  # Return top 3
    
    def _identify_demand_zones(self, lows, volumes, current_price):
        """Identify demand/support zones"""
        demand_zones = []
        
        if not lows or len(lows) < 5:
            return demand_zones
            ...
            if (lows[i] < lows[i-1] and lows[i] < lows[i-2] and 
                ...
                if price_level < current_price:
                    demand_zones.append({
                        'price': price_level,
                        'strength': volume_strength,
                        'distance': abs(current_price - price_level) / current_price
                    })
        
        # Sort by proximity to current price
        demand_zones.sort(key=lambda x: x['distance'])
        return demand_zones[:3]  # Return top 3
    
    def _determine_snd_signal(self, current_price, change_24h, volume_24h, supply_zones, demand_zones):
        """Determine trading signal based on SnD analysis"""
        try:
            ...
            # BUY signal logic (near demand zone)
            if nearest_demand and nearest_demand['distance'] < 0.02:  # Within 2%
                if change_24h > 0:  # Positive momentum
                    signal_type = 'BUY'
                    confidence = min(95, base_confidence + 20)
                    
                    # SnD-based entry and levels
                    entry_price = nearest_demand['price'] * 1.005  # Slightly above demand
                    stop_loss = nearest_demand['price'] * 0.995   # Below demand zone
                    
                    # Calculate TP based on nearest supply or R:R
                    if nearest_supply:
                        tp1 = nearest_supply['price'] * 0.995     # Just below supply
                        tp2 = nearest_supply['price'] * 1.005     # Through supply
                    else:
                        risk = abs(entry_price - stop_loss)
                        tp1 = entry_price + (risk * 1.5)         # 1.5:1 R:R
                        tp2 = entry_price + (risk * 2.5)         # 2.5:1 R:R
                    
                    reasoning = f"Strong demand zone at ${nearest_demand['price']:.6f} with bullish momentum"
            
            # SELL signal logic (near supply zone)
            elif nearest_supply and nearest_supply['distance'] < 0.02:  # Within 2%
                if change_24h < 0:  # Negative momentum
                    signal_type = 'SELL'
                    confidence = min(95, base_confidence + 20)
                    
                    # SnD-based entry and levels
                    entry_price = nearest_supply['price'] * 0.995   # Slightly below supply
                    stop_loss = nearest_supply['price'] * 1.005    # Above supply zone
                    
                    # Calculate TP based on nearest demand or R:R
                    if nearest_demand:
                        tp1 = nearest_demand['price'] * 1.005      # Just above demand
                        tp2 = nearest_demand['price'] * 0.995      # Through demand
                    else:
                        risk = abs(stop_loss - entry_price)
                        tp1 = entry_price - (risk * 1.5)          # 1.5:1 R:R
                        tp2 = entry_price - (risk * 2.5)          # 2.5:1 R:R
                    
                    reasoning = f"Strong supply zone at ${nearest_supply['price']:.6f} with bearish momentum"
            
            # Momentum-based signals (no clear zones)
            elif abs(change_24h) > 3 and volume_24h > 5000000:
                if change_24h > 3:  # Strong bullish momentum
                    signal_type = 'BUY'
                    ...
                    reasoning = f"Strong bullish momentum (+{change_24h:.1f}%) with high volume"
                
                elif change_24h < -3:  # Strong bearish momentum
                    signal_type = 'SELL'
                    confidence = min(85, base_confidence + 10)
                    ...
                    reasoning = f"Strong bearish momentum ({change_24h:.1f}%) with high volume"
