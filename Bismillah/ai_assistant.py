...
Try again in a few minutes for real-time data."""
    ...
    def _get_fallback_market_overview_with_error(self, language='id', error_message=''):
        """Fallback market overview with error details"""
        if language == 'id':
            return f"""🌍 **OVERVIEW PASAR CRYPTO** (Error Mode)
...
🔄 Coba command `/market` lagi dalam beberapa menit."""
            ...
            return f"""🌍 **CRYPTO MARKET OVERVIEW** (Error Mode)
...
🔄 Try `/market` command again in a few minutes."""
    ...
    def _extract_entry_zones(self, sd_analysis, current_price):
        ...
        try:
            ...
            if demand_zones:
                nearest_demand = min(demand_zones, key=lambda x: abs(x.get('price_level', 0) - current_price))
                distance = abs(nearest_demand.get('price_level', 0) - current_price) / current_price * 100
                
                if distance <= 2:  # Within 2% of demand zone
                    entry_zones['current_zone_type'] = 'near_demand'
                    entry_zones['strong_demand'] = {
                        'zone_high': nearest_demand.get('price_level', current_price) * 1.005,
                        'zone_low': nearest_demand.get('price_level', current_price) * 0.995
                    }
            
            # Find nearest supply zone
            if supply_zones:
                nearest_supply = min(supply_zones, key=lambda x: abs(x.get('price_level', 0) - current_price))
                distance = abs(nearest_supply.get('price_level', 0) - current_price) / current_price * 100
                
                if distance <= 2:  # Within 2% of supply zone
                    entry_zones['current_zone_type'] = 'near_supply'
                    entry_zones['strong_supply'] = {
                        'zone_high': nearest_supply.get('price_level', current_price) * 1.005,
                        'zone_low': nearest_supply.get('price_level', current_price) * 0.995
                    }
    ...
    def _analyze_news_sentiment(self, news_data, context='CRYPTO'):
        ...
        try:
            ...
            positive_keywords = ['bull', 'rise', 'surge', 'pump', 'gain', 'rally', 'high', 'adoption', 'breakthrough', 'bullish', 'up', 'green']
            negative_keywords = ['bear', 'fall', 'crash', 'dump', 'loss', 'decline', 'low', 'ban', 'hack', 'bearish', 'down', 'red']
            
            sentiment_score = 5  # Neutral base
            
            for news in news_data[:5]:  # Analyze first 5 news items
                title = news.get('title', '').lower()
    ...
    def get_comprehensive_analysis(self, symbol, futures_data, price_data, language='id', crypto_api=None):
        """Generate comprehensive analysis for analyze command"""
        from datetime import datetime
...
Tidak dapat mengakses data untuk analisis komprehensif {symbol}.
Silakan coba lagi nanti."""
                else:
                    return f"""❌ **Error: API unavailable**

Cannot access data for comprehensive analysis of {symbol}.
Please try again later."""
            ...
            try:
                ...
                news_data = []
                news_sentiment = {'score': 5, 'sentiment': 'neutral'}
            
            # Get supply/demand analysis
            try:
                sd_analysis = crypto_api.analyze_supply_demand(symbol)
                sd_score = sd_analysis.get('supply_demand_score', {}).get('score', 50) if 'error' not in sd_analysis else 50
            except:
                sd_score = 50
                sd_analysis = {'error': 'S&D analysis unavailable'}
...
📈 **Analisis Teknikal:**
• Supply/Demand Score: {sd_score}/100
• Sentiment Berita: {news_sentiment['sentiment'].title()} ({news_sentiment['score']}/10)

🎯 **Rekomendasi Trading:**"""

                # Generate trading recommendation
                if sd_score >= 70 and news_sentiment['score'] >= 6:
                    analysis += f"""
• **Bias**: BULLISH
• **Entry Zone**: ${current_price * 0.995:,.4f} - ${current_price * 1.005:,.4f}
• **Target**: ${current_price * 1.03:,.4f} (+3%)
• **Stop Loss**: ${current_price * 0.97:,.4f} (-3%)
• **Risk/Reward**: 1:1"""
                elif sd_score <= 30 and news_sentiment['score'] <= 4:
                    analysis += f"""
• **Bias**: BEARISH
• **Short Entry**: ${current_price * 0.995:,.4f} - ${current_price * 1.005:,.4f}
• **Target**: ${current_price * 0.97:,.4f} (-3%)
• **Stop Loss**: ${current_price * 1.03:,.4f} (+3%)
• **Risk/Reward**: 1:1"""
                else:
                    analysis += f"""
• **Bias**: NEUTRAL
• **Rekomendasi**: HOLD atau tunggu konfirmasi breakout
• **Watch Level**: ${current_price * 0.98:,.4f} (Support) | ${current_price * 1.02:,.4f} (Resistance)"""
...
📰 **Konteks Berita:**
• {latest_news.get('title', 'N/A')[:80]}...
• Impact: {news_sentiment['sentiment'].title()} pada sentiment"""

                analysis += f"""

📡 **Sumber Data:** Binance + CoinGecko + CryptoNews
🕐 **Update:** {datetime.now().strftime('%H:%M:%S WIB')}

⚠️ **Risk Warning:**
Analisis ini hanya untuk edukasi. Selalu gunakan proper risk management!"""
...
📈 **Technical Analysis:**
• Supply/Demand Score: {sd_score}/100
• News Sentiment: {news_sentiment['sentiment'].title()} ({news_sentiment['score']}/10)
...
• **Risk/Reward**: 1:1"""
                    ...
                    analysis += f"""
• **Bias**: NEUTRAL
• **Recommendation**: HOLD or wait for breakout confirmation
• **Watch Level**: ${current_price * 0.98:,.4f} (Support) | ${current_price * 1.02:,.4f} (Resistance)"""
...
📰 **News Context:**
• {latest_news.get('title', 'N/A')[:80]}...
• Impact: {news_sentiment['sentiment'].title()} sentiment"""

                analysis += f"""

📡 **Data Sources:** Binance + CoinGecko + CryptoNews
🕐 **Update:** {datetime.now().strftime('%H:%M:%S UTC')}
...
This analysis is for educational purposes only. Always use proper risk management!"""
            ...
            if language == 'id':
                return f"❌ Terjadi kesalahan dalam analisis komprehensif: {str(e)[:100]}"
            else:
                return f"❌ Error in comprehensive analysis: {str(e)[:100]}"
