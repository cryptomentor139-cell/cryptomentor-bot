...
🚀 **Semua analisis menggunakan data real-time dari multiple API!**"""
    ...
    def get_futures_analysis(self, symbol, timeframe, language='id', crypto_api=None):
        """Generate Smart Money Concept (SMC) based futures analysis - Premium feature only"""
        try:
            ...
            # Apply Smart Money Concept analysis
            smc_analysis = self._analyze_smart_money_concept(symbol, market_data, timeframe)
            
            # Generate SMC-based trading levels
            trading_levels = self._calculate_smc_trading_levels(market_data['price'], smc_analysis, timeframe)
            
            # Format SMC output
            return self._format_smc_futures_output(symbol, timeframe, market_data, smc_analysis, trading_levels, language)
    ...
    def _get_smc_market_data(self, symbol, timeframe, crypto_api):
        """Get comprehensive market data for Smart Money Concept analysis"""
        try:
            # Get basic market data
            basic_data = self._get_market_data(symbol, timeframe, crypto_api)
            
            # Get candlestick data for structure analysis
            candlestick_data = crypto_api.get_binance_candlestick(symbol, timeframe, 100) if crypto_api else {}
            
            # Get higher timeframe for trend filter
            htf_timeframe = self._get_higher_timeframe(timeframe)
            htf_candlesticks = crypto_api.get_binance_candlestick(symbol, htf_timeframe, 50) if crypto_api else {}
            
            # Add SMC-specific data
            basic_data.update({
                'candlesticks': candlestick_data.get('candlesticks', []),
                'htf_candlesticks': htf_candlesticks.get('candlesticks', []),
            ...
            print(f"❌ Error getting SMC market data: {e}")
            return self._get_market_data(symbol, timeframe, crypto_api)
...
⏰ **Update**: {datetime.now().strftime('%H:%M:%S UTC')}"""
    ...
    def _get_higher_timeframe(self, timeframe):
        ...
        return htf_mapping.get(timeframe, '4h')

    def _analyze_smart_money_concept(self, symbol, market_data, timeframe):
        """Comprehensive Smart Money Concept analysis"""
        try:
            print(f"🧠 Running SMC analysis for {symbol}")
            
            candlesticks = market_data.get('candlesticks', [])
            htf_candlesticks = market_data.get('htf_candlesticks', [])
            ...
            if len(candlesticks) < 50:
                smc_analysis['factors'].append("⚠️ Insufficient data for SMC analysis")
                return smc_analysis
            
            # 1. Trend Filter Analysis (Higher Timeframe)
            trend_filter = self._analyze_trend_filter(htf_candlesticks, current_price)
            smc_analysis['trend_filter'] = trend_filter['direction']
            smc_analysis['factors'].append(f"📊 HTF Trend: {trend_filter['direction']} ({trend_filter['strength']})")
            
            # 2. Market Structure Analysis (BOS & ChoCH)
            structure_analysis = self._detect_market_structure(candlesticks, current_price)
            smc_analysis.update(structure_analysis)
            
            # 3. Supply & Demand Zones (SMC approach)
            snd_zones = self._identify_smc_snd_zones(candlesticks, current_price)
            smc_analysis['snd_zones'] = snd_zones
            
            # 4. Fair Value Gap Detection
            fvg_zones = self._detect_fair_value_gaps(candlesticks, current_price)
            smc_analysis['fvg_zones'] = fvg_zones
            
            # 5. Price Action Analysis
            price_action = self._analyze_price_action_rejection(candlesticks)
            
            # 6. Calculate SMC Confidence Score
            confidence_score = self._calculate_smc_confidence(smc_analysis, price_action, trend_filter)
            smc_analysis['confidence'] = confidence_score
            
            # 7. Determine Final Direction
            final_direction = self._determine_smc_direction(smc_analysis, trend_filter)
            smc_analysis['direction'] = final_direction
            ...
            return {
                'direction': 'HOLD',
                'confidence': 0,
                'factors': [f"❌ SMC Analysis Error: {str(e)[:100]}"],
                'market_structure': 'ERROR'
            }

    def _analyze_trend_filter(self, htf_candlesticks, current_price):
        """Analyze higher timeframe trend using EMA 50/200 concept"""
        try:
            if len(htf_candlesticks) < 200:
                return {'direction': 'NEUTRAL', 'strength': 'WEAK'}
            
            # Calculate EMA-like values using simple moving averages
            closes = [float(candle[4]) for candle in htf_candlesticks[-200:]]
            ...
            if current_price > ema_50 > ema_200:
                direction = 'BULLISH'
                strength = 'STRONG' if current_price > ema_50 * 1.02 else 'MODERATE'
            elif current_price < ema_50 < ema_200:
                direction = 'BEARISH'
                strength = 'STRONG' if current_price < ema_50 * 0.98 else 'MODERATE'
            elif ema_50 > ema_200:
                direction = 'BULLISH'
                strength = 'WEAK'
            elif ema_50 < ema_200:
            ...
            print(f"❌ Error in trend filter: {e}")
            return {'direction': 'NEUTRAL', 'strength': 'WEAK'}

    def _detect_market_structure(self, candlesticks, current_price):
        ...
        try:
            ...
            # Convert candlesticks to OHLC
            highs = [float(candle[2]) for candle in candlesticks[-50:]]
            lows = [float(candle[3]) for candle in candlesticks[-50:]]
            closes = [float(candle[4]) for candle in candlesticks[-50:]]
            ...
            if swing_highs and swing_lows:
                recent_high = max(swing_highs[-3:]) if len(swing_highs) >= 3 else max(highs[-10:])
                recent_low = min(swing_lows[-3:]) if len(swing_lows) >= 3 else min(lows[-10:])
                
                # Bullish BOS: Price breaks above recent swing high
                if current_price > recent_high * 1.001:  # 0.1% buffer
                    structure_analysis['bos_detected'] = True
                    structure_analysis['market_structure'] = 'BULLISH_BOS'
                    structure_analysis['factors'].append(f"🔥 Bullish BOS detected (Break: ${recent_high:.4f})")
                
                # Bearish BOS: Price breaks below recent swing low
                elif current_price < recent_low * 0.999:  # 0.1% buffer
                    structure_analysis['bos_detected'] = True
                    structure_analysis['market_structure'] = 'BEARISH_BOS'
                    structure_analysis['factors'].append(f"🔥 Bearish BOS detected (Break: ${recent_low:.4f})")
            
            # Detect ChoCH (Change of Character)
            choch_detected = self._detect_change_of_character(highs, lows, closes)
            if choch_detected:
                structure_analysis['choch_detected'] = True
                structure_analysis['factors'].append("⚡ Change of Character detected")
            
            return structure_analysis
            
        except Exception as e:
            print(f"❌ Error detecting market structure: {e}")
            return {
                'market_structure': 'ERROR',
                'bos_detected': False,
                'choch_detected': False,
                'factors': [f"❌ Structure analysis error"]
            }

    def _find_swing_points(self, prices, point_type='high'):
        ...
        try:
            ...
            for i in range(lookback, len(prices) - lookback):
                if point_type == 'high':
                    is_swing = all(prices[i] >= prices[i-j] for j in range(1, lookback+1)) and \
    ...
    def _detect_change_of_character(self, highs, lows, closes):
        """Detect Change of Character pattern"""
        try:
            if len(closes) < 10:
                return False
            
            # Simple ChoCH detection: trend reversal pattern
            recent_closes = closes[-10:]
            
            # Check for bullish to bearish ChoCH
            bullish_trend = all(recent_closes[i] > recent_closes[i-1] for i in range(1, 5))
            bearish_reversal = all(recent_closes[i] < recent_closes[i-1] for i in range(6, 10))
            
            # Check for bearish to bullish ChoCH
            bearish_trend = all(recent_closes[i] < recent_closes[i-1] for i in range(1, 5))
            bullish_reversal = all(recent_closes[i] > recent_closes[i-1] for i in range(6, 10))
            
            return (bullish_trend and bearish_reversal) or (bearish_trend and bullish_reversal)
            
        except Exception:
            return False

    def _identify_smc_snd_zones(self, candlesticks, current_price):
        """Identify Supply & Demand zones using SMC methodology"""
        try:
            if len(candlesticks) < 20:
                return []
            ...
            # Find demand zones (areas where price moved up strongly)
            for i in range(5, len(ohlcv_data) - 5):
                # Look for strong bullish moves
                current_candle = ohlcv_data[i]
                prev_candles = ohlcv_data[i-3:i]
                next_candles = ohlcv_data[i+1:i+4]
                
                # Check for strong bullish impulse
                if self._is_strong_bullish_impulse(prev_candles, current_candle, next_candles):
                    zone_low = min(c[3] for c in prev_candles)  # Lowest low of base
                    zone_high = max(c[2] for c in prev_candles)  # Highest high of base
                    
                    # Distance from current price
                    distance = abs(current_price - ((zone_low + zone_high) / 2)) / current_price * 100
                    
                    if distance < 10:  # Within 10% of current price
                        snd_zones.append({
                            'type': 'DEMAND',
                            'low': zone_low,
                            'high': zone_high,
                            ...
                            'distance': distance,
                            'strength': self._calculate_zone_strength(prev_candles, current_candle)
                        })
            
            # Find supply zones (areas where price moved down strongly)
            for i in range(5, len(ohlcv_data) - 5):
                current_candle = ohlcv_data[i]
                prev_candles = ohlcv_data[i-3:i]
                next_candles = ohlcv_data[i+1:i+4]
                ...
                if self._is_strong_bearish_impulse(prev_candles, current_candle, next_candles):
                    zone_low = min(c[3] for c in prev_candles)
                    zone_high = max(c[2] for c in prev_candles)
            ...
            snd_zones.sort(key=lambda x: x['strength'], reverse=True)
            return snd_zones[:5]  # Top 5 zones
            
        except Exception as e:
            print(f"❌ Error identifying SMC SnD zones: {e}")
            return []

    def _is_strong_bullish_impulse(self, prev_candles, current_candle, next_candles):
        """Check if there's a strong bullish impulse from the zone"""
        try:
            # Calculate the impulse strength
            base_range = max(c[2] for c in prev_candles) - min(c[3] for c in prev_candles)
            impulse_range = max(c[2] for c in next_candles) - current_candle[1]  # From zone open to impulse high
            
            # Strong impulse should be at least 2x the base range
            return impulse_range > base_range * 1.5 and len([c for c in next_candles if c[4] > c[1]]) >= 2
            
        except Exception:
            return False

    def _is_strong_bearish_impulse(self, prev_candles, current_candle, next_candles):
        """Check if there's a strong bearish impulse from the zone"""
        try:
            base_range = max(c[2] for c in prev_candles) - min(c[3] for c in prev_candles)
            impulse_range = current_candle[1] - min(c[3] for c in next_candles)  # From zone open to impulse low
    ...
    def _calculate_zone_strength(self, prev_candles, current_candle):
        """Calculate strength of SnD zone"""
        try:
            # Factor in volume, range, and time in zone
            avg_volume = sum(c[4] for c in prev_candles) / len(prev_candles)
            current_volume = current_candle[4]
            
            volume_factor = min(2.0, current_volume / avg_volume) if avg_volume > 0 else 1.0
            
            # Range factor
            zone_range = max(c[2] for c in prev_candles) - min(c[3] for c in prev_candles)
            range_factor = min(2.0, zone_range / prev_candles[0][1] * 100) if prev_candles[0][1] > 0 else 1.0
            
            strength = (volume_factor + range_factor) * 25  # Scale to 0-100
            return min(100, strength)
            
        except Exception:
            return 50

    def _detect_fair_value_gaps(self, candlesticks, current_price):
        ...
        try:
            ...
            for i in range(1, len(candlesticks) - 1):
                candle1 = [float(x) for x in candlesticks[i-1][1:6]]  # [open, high, low, close, volume]
                candle2 = [float(x) for x in candlesticks[i][1:6]]
                candle3 = [float(x) for x in candlesticks[i+1][1:6]]
                
                # Bullish FVG: gap between candle1 high and candle3 low
                if candle2[4] > candle2[1] and candle1[2] < candle3[3]:  # Bullish candle2 and gap exists
                    gap_low = candle1[2]
                    gap_high = candle3[3]
                    gap_mid = (gap_low + gap_high) / 2
                    
                    # Check if FVG is near current price
                    distance = abs(current_price - gap_mid) / current_price * 100
                    
                    if distance < 5 and gap_high > gap_low:  # Valid gap within 5%
                        fvg_zones.append({
                            'type': 'BULLISH_FVG',
                            'low': gap_low,
                            'high': gap_high,
                            'mid': gap_mid,
                            'distance': distance,
                            'filled': current_price < gap_low  # Check if already filled
                        })
                
                # Bearish FVG: gap between candle1 low and candle3 high
                elif candle2[4] < candle2[1] and candle1[3] > candle3[2]:  # Bearish candle2 and gap exists
                    gap_low = candle3[2]
                    gap_high = candle1[3]
                    gap_mid = (gap_low + gap_high) / 2
                    ...
                    if distance < 5 and gap_high > gap_low:
                        fvg_zones.append({
                            'type': 'BEARISH_FVG',
                            'low': gap_low,
                            'high': gap_high,
            ...
            # Return unfilled FVGs closest to price
            unfilled_fvgs = [fvg for fvg in fvg_zones if not fvg['filled']]
            unfilled_fvgs.sort(key=lambda x: x['distance'])
            
            return unfilled_fvgs[:3]  # Top 3 closest unfilled FVGs
            
        except Exception as e:
            print(f"❌ Error detecting FVGs: {e}")
            return []

    def _analyze_price_action_rejection(self, candlesticks):
        """Analyze recent price action for rejection patterns"""
        try:
            if len(candlesticks) < 5:
                return {'rejection_detected': False, 'type': None}
            
            # Get last 3 candles
            recent_candles = candlesticks[-3:]
            last_candle = [float(x) for x in recent_candles[-1][1:6]]
            
            # Check for doji pattern
            body_size = abs(last_candle[4] - last_candle[1])  # close - open
            total_range = last_candle[2] - last_candle[3]     # high - low
            
            is_doji = body_size < (total_range * 0.1) if total_range > 0 else False
            
            # Check for hammer/hanging man
            upper_shadow = last_candle[2] - max(last_candle[1], last_candle[4])
            lower_shadow = min(last_candle[1], last_candle[4]) - last_candle[3]
            ...
            if is_doji:
                return {'rejection_detected': True, 'type': 'DOJI'}
            elif is_hammer:
                return {'rejection_detected': True, 'type': 'HAMMER'}
            elif is_shooting_star:
                return {'rejection_detected': True, 'type': 'SHOOTING_STAR'}
            else:
                return {'rejection_detected': False, 'type': None}
    ...
    def _calculate_smc_confidence(self, smc_analysis, price_action, trend_filter):
        """Calculate SMC confidence score based on multiple factors"""
        try:
            confidence = 0
            
            # Base confidence for valid zones
            valid_demand_zones = [z for z in smc_analysis['snd_zones'] if z['type'] == 'DEMAND' and z['distance'] < 3]
            valid_supply_zones = [z for z in smc_analysis['snd_zones'] if z['type'] == 'SUPPLY' and z['distance'] < 3]
            
            if valid_demand_zones or valid_supply_zones:
                confidence += 30  # SnD valid = +30%
                smc_analysis['factors'].append(f"✅ Valid SnD zones found ({len(valid_demand_zones + valid_supply_zones)})")
            
            # BOS detection
            if smc_analysis['bos_detected']:
                confidence += 30  # BOS valid = +30%
                smc_analysis['factors'].append("✅ Break of Structure confirmed")
            
            # FVG detection
            unfilled_fvgs = [fvg for fvg in smc_analysis['fvg_zones'] if not fvg['filled']]
            if unfilled_fvgs:
                confidence += 30  # FVG found and not filled = +30%
                smc_analysis['factors'].append(f"✅ {len(unfilled_fvgs)} unfilled FVG(s) detected")
            
            # Price action rejection
            if price_action['rejection_detected']:
                confidence += 10  # Price action rejection = +10%
                smc_analysis['factors'].append(f"✅ Price rejection pattern: {price_action['type']}")
            
            # Trend filter alignment
            if trend_filter['strength'] in ['STRONG', 'MODERATE']:
                trend_bonus = 15 if trend_filter['strength'] == 'STRONG' else 10
                confidence += trend_bonus
                smc_analysis['factors'].append(f"✅ Strong trend alignment: {trend_filter['direction']}")
            
            return min(95, confidence)  # Cap at 95%
            
        except Exception as e:
            print(f"❌ Error calculating SMC confidence: {e}")
            return 50

    def _determine_smc_direction(self, smc_analysis, trend_filter):
        """Determine final trading direction based on SMC analysis"""
        try:
            ...
            bullish_bos = smc_analysis.get('market_structure') == 'BULLISH_BOS'
            bullish_trend = trend_filter['direction'] in ['BULLISH', 'NEUTRAL']
            bullish_fvg = any(fvg['type'] == 'BULLISH_FVG' and not fvg['filled'] for fvg in smc_analysis['fvg_zones'])
            
            # Bearish setup criteria  
            bearish_bos = smc_analysis.get('market_structure') == 'BEARISH_BOS'
            bearish_trend = trend_filter['direction'] in ['BEARISH', 'NEUTRAL']
            bearish_fvg = any(fvg['type'] == 'BEARISH_FVG' and not fvg['filled'] for fvg in smc_analysis['fvg_zones'])
            
            # Decision logic
            if valid_demand_zones and bullish_bos and bullish_trend:
                return 'LONG'
            elif valid_supply_zones and bearish_bos and bearish_trend:
                return 'SHORT'
            elif bullish_fvg and bullish_bos and bullish_trend:
                return 'LONG'  
            elif bearish_fvg and bearish_bos and bearish_trend:
                return 'SHORT'
            else:
                return 'HOLD'
                
        except Exception as e:
            print(f"❌ Error determining SMC direction: {e}")
            return 'HOLD'

    def _calculate_smc_trading_levels(self, current_price, smc_analysis, timeframe):
        """Calculate trading levels based on SMC analysis"""
        try:
            ...
            best_zone = None
            if direction == 'LONG':
                demand_zones = [z for z in smc_analysis['snd_zones'] if z['type'] == 'DEMAND']
                if demand_zones:
                    best_zone = min(demand_zones, key=lambda x: x['distance'])
            else:  # SHORT
                supply_zones = [z for z in smc_analysis['snd_zones'] if z['type'] == 'SUPPLY']
                if supply_zones:
                    best_zone = min(supply_zones, key=lambda x: x['distance'])
            
            # Calculate levels
            if direction == 'LONG':
                if best_zone:
                    entry = best_zone['mid']  # Enter at zone middle
                    sl = best_zone['low'] * 0.995  # SL below zone
                else:
                    entry = current_price * 0.999
                    sl = current_price * 0.985
                
                # TP based on risk-reward (1:2 and 1:3 ratios)
                risk = abs(entry - sl)
                tp1 = entry + (risk * 2)   # 1:2 RR
                tp2 = entry + (risk * 3)   # 1:3 RR
                
            else:  # SHORT
                if best_zone:
                    entry = best_zone['mid']
                    sl = best_zone['high'] * 1.005  # SL above zone
                else:
            ...
            rr1 = abs(tp1 - entry) / risk_amount if risk_amount > 0 else 2.0
            rr2 = abs(tp2 - entry) / risk_amount if risk_amount > 0 else 3.0
            ...
            return {
                ...
                'zone_entry': best_zone is not None
            }
            
        except Exception as e:
            print(f"❌ Error calculating SMC trading levels: {e}")
            # Fallback levels
            return {
                'entry': current_price,
    ...
    def _format_smc_futures_output(self, symbol, timeframe, market_data, smc_analysis, trading_levels, language='id'):
        """Format SMC analysis output for Telegram"""
        try:
            price = market_data['price']
            direction = smc_analysis['direction']
            confidence = smc_analysis['confidence']
            factors = smc_analysis['factors']
            ...
            # Direction emoji and confidence emoji
            if direction == 'HOLD':
                direction_emoji = "⏸️"
                confidence_emoji = "⚠️"
            elif direction == 'SHORT':
            ...
            if language == 'id':
                if direction == 'HOLD':
                    message = f"""🧠 **SMART MONEY CONCEPT ANALYSIS - {symbol.upper()} ({timeframe})**

💰 **Harga**: {format_price(price)} {market_data['price_emoji']}
📡 **Source**: {market_data['price_source']}

{direction_emoji} **SMC SIGNAL**: {direction} {confidence_emoji}
📊 **Confidence**: {confidence:.0f}%
🏗️ **Market Structure**: {smc_analysis.get('market_structure', 'UNKNOWN')}

⏸️ **REKOMENDASI HOLD - TUNGGU SETUP YANG LEBIH BAIK:**
• Struktur pasar belum memberikan sinyal SMC yang valid
• Tidak ada konfirmasi BOS + pullback ke zona valid
• Risk/reward tidak optimal untuk entry saat ini
• Tunggu pembentukan struktur yang lebih jelas
...
💡 **YANG HARUS DITUNGGU:**
• Break of Structure (BOS) yang jelas
• Pullback ke zona Supply/Demand yang valid
• Konfirmasi FVG yang belum terisi
• Price action rejection di zona kunci

⚠️ **PENTING - SMART MONEY RULES:**
• JANGAN paksa entry tanpa struktur yang valid
• Tunggu konfirmasi semua elemen SMC
• Patience adalah kunci dalam SMC trading
• Setup yang buruk = kerugian pasti
...
💰 **SMC TRADING LEVELS:**
📍 **Entry**: {format_price(trading_levels['entry'])} {'(di zona SnD)' if trading_levels.get('zone_entry') else '(market entry)'}
🎯 **TP1**: {format_price(trading_levels['tp1'])} (RR: {trading_levels['rr_ratio1']:.1f}:1)
🎯 **TP2**: {format_price(trading_levels['tp2'])} (RR: {trading_levels['rr_ratio2']:.1f}:1)
🛡️ **Stop Loss**: {format_price(trading_levels['sl'])}
...
🧠 **ANALISIS SMC:**"""
                    ...
                    # Show SnD zones if available
                    if smc_analysis['snd_zones']:
                        relevant_zones = [z for z in smc_analysis['snd_zones'] if z['distance'] < 5][:2]
                        message += f"\n\n🎯 **ZONA AKTIF:**"
                        for zone in relevant_zones:
                            zone_type_emoji = "🟢" if zone['type'] == 'DEMAND' else "🔴"
                            message += f"\n• {zone_type_emoji} {zone['type']}: {format_price(zone['low'])} - {format_price(zone['high'])} (Distance: {zone['distance']:.1f}%)"
                    
                    # Show FVG if available
                    if smc_analysis['fvg_zones']:
                        unfilled_fvgs = [fvg for fvg in smc_analysis['fvg_zones'] if not fvg['filled']][:2]
                        if unfilled_fvgs:
                            message += f"\n\n⚡ **FAIR VALUE GAPS:**"
                            for fvg in unfilled_fvgs:
                                fvg_emoji = "🟢" if fvg['type'] == 'BULLISH_FVG' else "🔴"
                                message += f"\n• {fvg_emoji} {fvg['type']}: {format_price(fvg['low'])} - {format_price(fvg['high'])}"
                    
                    message += f"""

⚡ **STRATEGI SMC {timeframe.upper()}:**
• **Risk Level**: {trading_levels['risk_level'].upper()}
• **Position Size**: {"2-3%" if confidence >= 80 else "1-2%" if confidence >= 70 else "0.5-1%"} dari modal
• **Entry Method**: {"Limit order di zona" if trading_levels.get('zone_entry') else "Market entry dengan konfirmasi"}
• **Take Profit**: 50% di TP1, 50% di TP2

🛡️ **SMC RISK MANAGEMENT:**
• Entry HANYA setelah konfirmasi price action di zona
• SL WAJIB di luar zona SnD (tidak boleh di dalam zona)
• Move SL ke breakeven setelah TP1 tercapai
• Exit segera jika struktur berubah (ChoCH detected)

⚠️ **RULES SMC KETAT:**
• Tunggu pullback ke zona setelah BOS
• Validasi dengan multiple timeframe
• Jangan trade melawan struktur utama
• Respect semua level Supply & Demand

⏰ **Update**: {current_time}
🔄 **Next Structure Check**: Monitor untuk BOS/ChoCH baru"""
...
⏸️ **HOLD RECOMMENDATION - WAIT FOR BETTER SETUP:**
• Market structure hasn't provided valid SMC signals
• No BOS confirmation + pullback to valid zones
• Risk/reward not optimal for entry
• Wait for clearer structure formation
...
• Pullback to valid Supply/Demand zones
• Unfilled Fair Value Gap confirmation
• Price action rejection at key zones

⚠️ **IMPORTANT - SMART MONEY RULES:**
• DON'T force entry without valid structure
• Wait for all SMC elements confirmation
• Patience is key in SMC trading
• Poor setups = guaranteed losses
...
💰 **SMC TRADING LEVELS:**
📍 **Entry**: {format_price(trading_levels['entry'])} {'(at SnD zone)' if trading_levels.get('zone_entry') else '(market entry)'}
🎯 **TP1**: {format_price(trading_levels['tp1'])} (RR: {trading_levels['rr_ratio1']:.1f}:1)
🎯 **TP2**: {format_price(trading_levels['tp2'])} (RR: {trading_levels['rr_ratio2']:.1f}:1)
🛡️ **Stop Loss**: {format_price(trading_levels['sl'])}
...
⚡ **SMC STRATEGY {timeframe.upper()}:**
• **Risk Level**: {trading_levels['risk_level'].upper()}
• **Position Size**: {"2-3%" if confidence >= 80 else "1-2%" if confidence >= 70 else "0.5-1%"} of capital
• **Entry Method**: {"Limit order at zone" if trading_levels.get('zone_entry') else "Market entry with confirmation"}
• **Take Profit**: 50% at TP1, 50% at TP2

🛡️ **SMC RISK MANAGEMENT:**
• Entry ONLY after price action confirmation at zone
• SL MANDATORY outside SnD zone (never inside zone)
• Move SL to breakeven after TP1 hit
• Exit immediately if structure changes (ChoCH detected)

⚠️ **STRICT SMC RULES:**
• Wait for pullback to zone after BOS
• Validate with multiple timeframes
• Don't trade against main structure
• Respect all Supply & Demand levels

⏰ **Update**: {current_time}
🔄 **Next Structure Check**: Monitor for new BOS/ChoCH"""
            
            return message
            
        except Exception as e:
            print(f"❌ Error formatting SMC output: {e}")
            return f"❌ Error formatting SMC analysis: {str(e)}"
...
• {timeframe} timeframe suitable for {'scalping' if timeframe in ['15m', '30m'] else 'swing trading'}"""
    ...
    def generate_futures_signals(self, language='id', crypto_api=None):
        """Generate Smart Money Concept (SMC) futures signals - Premium feature only"""
        try:
            print(f"🧠 Generating SMC futures signals for premium users")
            ...
            for symbol in target_symbols:
                try:
                    ...
                    smc_signal = self._get_smc_signal_analysis(symbol, '1h', crypto_api, language)
                    if smc_signal:
                        smc_signals.append(smc_signal)
    ...
    def _get_smc_signal_analysis(self, symbol, timeframe, crypto_api, language='id'):
        """Get comprehensive SMC analysis for a single symbol"""
        try:
            ...
            if 'error' in market_data.get('coinapi_data', {}):
                return None
            ...
            trading_levels = self._calculate_smc_trading_levels(current_price, smc_analysis, timeframe)
            
            # Only return signals with reasonable confidence
            if smc_analysis['confidence'] < 60:
                return None
            ...
            return {
                ...
                'direction': smc_analysis['direction'],
                'confidence': smc_analysis['confidence'],
                'market_structure': smc_analysis.get('market_structure', 'UNKNOWN'),
                'trading_levels': trading_levels,
                'key_factors': smc_analysis['factors'][:3],  # Top 3 factors
                'snd_zones': len([z for z in smc_analysis['snd_zones'] if z['distance'] < 5]),
                'fvg_zones': len([f for f in smc_analysis['fvg_zones'] if not f['filled']])
            }
            
        except Exception as e:
            print(f"❌ Error in SMC signal analysis for {symbol}: {e}")
            return None

    def _format_smc_signals_output(self, smc_signals, language='id'):
        """Format SMC signals output for multiple symbols"""
        if not smc_signals:
            if language == 'id':
                return "❌ Tidak ada sinyal SMC yang memenuhi kriteria minimum. Semua market dalam kondisi HOLD - tunggu struktur yang lebih jelas."
            else:
                return "❌ No SMC signals meet minimum criteria. All markets in HOLD condition - wait for clearer structure."
        
        current_time = datetime.now().strftime('%H:%M:%S WIB')
        
        # Separate signals by direction
        long_signals = [s for s in smc_signals if s['direction'] == 'LONG']
        short_signals = [s for s in smc_signals if s['direction'] == 'SHORT']
        hold_signals = [s for s in smc_signals if s['direction'] == 'HOLD']
        ...
        if language == 'id':
            header = f"""🧠 **SMART MONEY CONCEPT SIGNALS**
⏰ {current_time} | 📊 TOP {len(smc_signals)} COINS ANALYSIS

💡 **METODE**: Smart Money Concept (BOS, SnD, FVG Analysis)
🎯 **MINIMUM CONFIDENCE**: 60% untuk memberikan sinyal
...
"""
            ...
            if long_signals:
                formatted_signals.append("🟢 **LONG SETUPS:**")
                for i, signal in enumerate(long_signals, 1):
                    ...
                    signal_text = f"""**{i}. {symbol} LONG** 🟢 - Confidence: {confidence:.0f}%
💰 Entry: {format_price(levels['entry'])} | TP1: {format_price(levels['tp1'])} | TP2: {format_price(levels['tp2'])}
🛡️ SL: {format_price(levels['sl'])} | RR: {levels['rr_ratio2']:.1f}:1
🏗️ Structure: {structure}
🎯 SnD Zones: {signal['snd_zones']} | FVG: {signal['fvg_zones']}"""
            ...
            if short_signals:
                ...
                for i, signal in enumerate(short_signals, 1):
                    ...
                    signal_text = f"""**{i}. {symbol} SHORT** 🔴 - Confidence: {confidence:.0f}%
💰 Entry: {format_price(levels['entry'])} | TP1: {format_price(levels['tp1'])} | TP2: {format_price(levels['tp2'])}
🛡️ SL: {format_price(levels['sl'])} | RR: {levels['rr_ratio2']:.1f}:1
🏗️ Structure: {structure}
🎯 SnD Zones: {signal['snd_zones']} | FVG: {signal['fvg_zones']}"""
...
1️⃣ **Entry Rules:**
   • Entry HANYA setelah BOS + pullback ke zona valid
   • Tunggu price action rejection di zona SnD/FVG
   • Konfirmasi dengan multiple timeframe analysis

2️⃣ **Risk Management SMC:**
   • SL WAJIB di luar zona SnD (NEVER inside zone)
   • Position size max 2% untuk high confidence (80%+)
   • Position size max 1% untuk medium confidence (60-79%)
   • Move SL ke BE setelah TP1 tercapai

3️⃣ **Structure Monitoring:**
   • Exit segera jika ChoCH (Change of Character) terdeteksi
   • Monitor pembentukan struktur baru
   • Respect semua level Supply & Demand

4️⃣ **Market Context:**
   • Prioritaskan trading dengan arah trend utama
   • Hindari counter-trend kecuali struktur sangat kuat
   • Tunggu konfirmasi sebelum entry

📊 **Data Sources**: CoinAPI + Binance + SMC Structure Analysis
⚠️ **PREMIUM FEATURE**: SMC Analysis hanya untuk user premium
🎯 **Success Rate**: SMC methodology memiliki win rate tinggi dengan proper execution

💡 **Pro Tip SMC**: "Patience dalam menunggu setup yang perfect adalah kunci sukses Smart Money trading"

🔄 **Next Update**: Real-time structure monitoring"""
            
        else:  # English
            header = f"""🧠 **SMART MONEY CONCEPT SIGNALS**
⏰ {current_time} | 📊 TOP {len(smc_signals)} COINS ANALYSIS

💡 **METHOD**: Smart Money Concept (BOS, SnD, FVG Analysis)
🎯 **MINIMUM CONFIDENCE**: 60% to provide signals
...
1️⃣ **Entry Rules:**
   • Entry ONLY after BOS + pullback to valid zone
   • Wait for price action rejection at SnD/FVG zones
   • Confirm with multiple timeframe analysis

2️⃣ **SMC Risk Management:**
   • SL MANDATORY outside SnD zone (NEVER inside zone)
   • Max position size 2% for high confidence (80%+)
   • Max position size 1% for medium confidence (60-79%)
   • Move SL to BE after TP1 hit

3️⃣ **Structure Monitoring:**
   • Exit immediately if ChoCH (Change of Character) detected
   • Monitor new structure formation
   • Respect all Supply & Demand levels

4️⃣ **Market Context:**
   • Prioritize trading with main trend direction
   • Avoid counter-trend unless structure very strong
   • Wait for confirmation before entry

📊 **Data Sources**: CoinAPI + Binance + SMC Structure Analysis
⚠️ **PREMIUM FEATURE**: SMC Analysis only for premium users
🎯 **Success Rate**: SMC methodology has high win rate with proper execution

💡 **SMC Pro Tip**: "Patience in waiting for perfect setup is the key to Smart Money trading success"
