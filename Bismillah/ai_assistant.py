...
Silakan coba lagi dalam beberapa menit atau pilih timeframe lain."""
    ...
    def analyze_snd_for_auto_signals(self, symbol, price_data, volume_data, futures_data):
        """Analyze Supply & Demand zones for automatic signal generation"""
        try:
            ...
            volume_analysis = self._analyze_volume_pressure(volume_24h, change_24h)
            
            # Analyze order flow imbalance
            order_flow = self._analyze_order_flow_imbalance(long_ratio, funding_rate)
            
            # Detect supply/demand zones
            zones = self._detect_snd_zones(current_price, change_24h, volume_24h)
            
            # Calculate market structure
            market_structure = self._analyze_market_structure(price_data, futures_data)
            
            # Generate S&D score
            snd_score = self._calculate_snd_score(volume_analysis, order_flow, zones, market_structure)
            
            # Determine signal recommendation
            signal_recommendation = self._generate_snd_signal_recommendation(snd_score, zones, current_price)
            
            return {
                'symbol': symbol,
                'current_price': current_price,
                'volume_pressure': volume_analysis,
                'order_imbalance': order_flow,
                'supply_demand_zones': zones,
                'market_structure': market_structure,
                'snd_score': snd_score,
                'signal_recommendation': signal_recommendation,
                'confidence_level': self._determine_confidence_level(snd_score, volume_analysis, order_flow),
                'risk_assessment': self._assess_snd_risks(snd_score, zones, current_price)
            }
            
        except Exception as e:
            print(f"Error in S&D analysis for {symbol}: {e}")
            return {'error': f'S&D analysis failed: {str(e)}'}

    def _analyze_volume_pressure(self, volume_24h, change_24h):
        """Analyze volume pressure for supply/demand analysis"""
        if volume_24h == 0:
            return {
                'pressure_type': 'insufficient_data',
                'pressure_strength': 0,
                'volume_level': 'unknown',
                'analysis': 'Volume data tidak tersedia'
            }
        ...
        # Analyze pressure type based on price movement and volume
        if abs(change_24h) > 5 and volume_level in ['high', 'very_high']:
            if change_24h > 0:
                pressure_type = 'strong_demand'
                pressure_strength = min(100, abs(change_24h) * 10 + volume_score * 10)
                analysis = 'Volume tinggi dengan price action bullish menunjukkan demand pressure kuat'
            else:
                pressure_type = 'strong_supply'
                pressure_strength = min(100, abs(change_24h) * 10 + volume_score * 10)
                analysis = 'Volume tinggi dengan price action bearish menunjukkan supply pressure kuat'
        elif abs(change_24h) > 2:
            if change_24h > 0:
                pressure_type = 'moderate_demand'
                pressure_strength = min(80, abs(change_24h) * 8 + volume_score * 8)
                analysis = 'Demand pressure moderat dengan konfirmasi volume'
            else:
                pressure_type = 'moderate_supply'
                pressure_strength = min(80, abs(change_24h) * 8 + volume_score * 8)
                analysis = 'Supply pressure moderat dengan konfirmasi volume'
        else:
            pressure_type = 'balanced'
            pressure_strength = 50
            analysis = 'Volume dan price action menunjukkan keseimbangan supply/demand'
    ...
    def _analyze_order_flow_imbalance(self, long_ratio, funding_rate):
        """Analyze order flow imbalance from futures data"""
        short_ratio = 100 - long_ratio
        
        # Determine imbalance type
        if long_ratio > 75:
            imbalance_type = 'extreme_long_bias'
            imbalance_strength = (long_ratio - 50) * 2  # Scale to 0-100
            analysis = 'Extreme long bias - potensi long squeeze tinggi'
        elif long_ratio > 65:
            imbalance_type = 'strong_long_bias'
            imbalance_strength = (long_ratio - 50) * 1.5
            analysis = 'Strong long bias - hati-hati dengan reversal'
        elif long_ratio < 25:
            imbalance_type = 'extreme_short_bias'
            imbalance_strength = (50 - long_ratio) * 2
            analysis = 'Extreme short bias - potensi short squeeze tinggi'
        elif long_ratio < 35:
            imbalance_type = 'strong_short_bias'
            imbalance_strength = (50 - long_ratio) * 1.5
            analysis = 'Strong short bias - potensi reversal ke atas'
        else:
            imbalance_type = 'balanced'
            imbalance_strength = 50
            analysis = 'Order flow relatif seimbang'
        
        # Factor in funding rate
        if abs(funding_rate) > 0.01:  # High funding rate
            if funding_rate > 0:
                analysis += ' | Funding rate tinggi - long position mahal'
            else:
                analysis += ' | Funding rate negatif - short position mahal'
        
        return {
            'imbalance_type': imbalance_type,
            'imbalance_strength': min(100, imbalance_strength),
            'long_ratio': long_ratio,
    ...
    def _detect_snd_zones(self, current_price, change_24h, volume_24h):
        """Detect key supply and demand zones"""
        zones = {
            'demand_zones': [],
            'supply_zones': [],
            'current_zone_context': 'neutral'
        }
        
        # Calculate potential zones based on price movement and volume
        price_volatility = abs(change_24h) / 100
        
        # Demand zones (support levels)
        if change_24h < 0:  # Price dropped, look for demand zones below
            demand_zone_1 = {
                'price_level': current_price * (1 - price_volatility * 0.5),
                'strength': min(100, volume_24h / 10000000 * 20),  # Volume-based strength
                'distance_from_current': -price_volatility * 50,
                'zone_type': 'demand',
                'zone_high': current_price * (1 - price_volatility * 0.4),
                'zone_low': current_price * (1 - price_volatility * 0.6)
            }
            
            demand_zone_2 = {
                'price_level': current_price * (1 - price_volatility * 1.0),
                'strength': min(80, volume_24h / 15000000 * 20),
                'distance_from_current': -price_volatility * 100,
                'zone_type': 'demand',
                'zone_high': current_price * (1 - price_volatility * 0.9),
                'zone_low': current_price * (1 - price_volatility * 1.1)
            }
            
            zones['demand_zones'] = [demand_zone_1, demand_zone_2]
        
        # Supply zones (resistance levels)
        if change_24h > 0:  # Price rose, look for supply zones above
            supply_zone_1 = {
                'price_level': current_price * (1 + price_volatility * 0.5),
                'strength': min(100, volume_24h / 10000000 * 20),
                'distance_from_current': price_volatility * 50,
                'zone_type': 'supply',
                'zone_high': current_price * (1 + price_volatility * 0.6),
                'zone_low': current_price * (1 + price_volatility * 0.4)
            }
            ...
            supply_zone_2 = {
                ...
                'distance_from_current': price_volatility * 100,
                'zone_type': 'supply',
                'zone_high': current_price * (1 + price_volatility * 1.1),
                'zone_low': current_price * (1 + price_volatility * 0.9)
            }
            
            zones['supply_zones'] = [supply_zone_1, supply_zone_2]
        
        # Determine current zone context
        if len(zones['demand_zones']) > 0:
            nearest_demand = min(zones['demand_zones'], key=lambda x: abs(x['distance_from_current']))
            if abs(nearest_demand['distance_from_current']) < 3:
                zones['current_zone_context'] = 'near_demand'
        
        if len(zones['supply_zones']) > 0:
            nearest_supply = min(zones['supply_zones'], key=lambda x: abs(x['distance_from_current']))
            if abs(nearest_supply['distance_from_current']) < 3:
                zones['current_zone_context'] = 'near_supply'
        
        return zones

    def _analyze_market_structure(self, price_data, futures_data):
        ...
        if volume_24h > 500000000:  # High volume confirmation
        ...
        analysis = f"Market structure menunjukkan {trend} trend dengan kualitas {structure_quality}"
        
        # Add sentiment analysis
        if (trend == 'bullish' and long_ratio > 70) or (trend == 'bearish' and long_ratio < 30):
            analysis += " - Sentiment ekstrem, hati-hati reversal"
        elif (trend == 'bullish' and long_ratio < 40) or (trend == 'bearish' and long_ratio > 60):
            analysis += " - Sentiment contrarian, potensi continuation"
    ...
    def _calculate_snd_score(self, volume_analysis, order_flow, zones, market_structure):
        """Calculate overall Supply & Demand score"""
        base_score = 50
        
        # Volume pressure factor (30% weight)
        volume_factor = 0
        if volume_analysis['pressure_type'] in ['strong_demand', 'moderate_demand']:
            volume_factor = volume_analysis['pressure_strength'] * 0.3
        elif volume_analysis['pressure_type'] in ['strong_supply', 'moderate_supply']:
            volume_factor = -volume_analysis['pressure_strength'] * 0.3
        
        # Order flow factor (25% weight)
        order_factor = 0
        if order_flow['imbalance_type'] in ['extreme_short_bias', 'strong_short_bias']:
            order_factor = order_flow['imbalance_strength'] * 0.25  # Contrarian bullish
        elif order_flow['imbalance_type'] in ['extreme_long_bias', 'strong_long_bias']:
            order_factor = -order_flow['imbalance_strength'] * 0.25  # Contrarian bearish
        
        # Zone proximity factor (25% weight)
        zone_factor = 0
        if zones['current_zone_context'] == 'near_demand':
            zone_factor = 15  # Bullish near demand
        elif zones['current_zone_context'] == 'near_supply':
            zone_factor = -15  # Bearish near supply
        
        # Market structure factor (20% weight)
        structure_factor = 0
        if market_structure['trend'] == 'bullish' and market_structure['quality'] == 'high':
            structure_factor = 20
        elif market_structure['trend'] == 'bearish' and market_structure['quality'] == 'high':
            structure_factor = -20
        elif market_structure['trend'] in ['bullish', 'bearish'] and market_structure['quality'] == 'medium':
            structure_factor = 10 if market_structure['trend'] == 'bullish' else -10
        
        # Calculate final score
        final_score = base_score + volume_factor + order_factor + zone_factor + structure_factor
        ...
        factors = []
        if volume_factor > 10:
            factors.append("Volume pressure mendukung demand")
        elif volume_factor < -10:
            factors.append("Volume pressure mendukung supply")
        
        if order_factor > 10:
            factors.append("Order flow imbalance bullish (contrarian)")
        elif order_factor < -10:
            factors.append("Order flow imbalance bearish (contrarian)")
        
        if zone_factor != 0:
            factors.append(f"Price {'near demand zone' if zone_factor > 0 else 'near supply zone'}")
        
        if structure_factor > 10:
            factors.append("Market structure bullish dengan volume konfirmasi")
        elif structure_factor < -10:
            factors.append("Market structure bearish dengan volume konfirmasi")
    ...
    def _generate_snd_signal_recommendation(self, snd_score, zones, current_price):
        """Generate trading signal recommendation based on S&D analysis"""
        score = snd_score['score']
        bias = snd_score['bias']
        confidence = snd_score['confidence']
        ...
        # Calculate entry levels based on zones
        if direction == 'LONG':
            # Look for demand zones for entry
            demand_zones = zones.get('demand_zones', [])
            if demand_zones:
                best_demand = max(demand_zones, key=lambda x: x['strength'])
                entry_price = best_demand['zone_high']
                stop_loss = best_demand['zone_low'] * 0.998
                take_profit = entry_price * 1.06  # 6% target
                entry_type = 'demand_zone_entry'
                logic = f"Entry at demand zone ${entry_price:.4f} dengan SL di bawah zone"
            else:
                entry_price = current_price * 0.999
                stop_loss = current_price * 0.96
                take_profit = current_price * 1.06
                entry_type = 'market_entry'
                logic = "Market entry dengan demand bias"
        
        elif direction == 'SHORT':
            # Look for supply zones for entry
            supply_zones = zones.get('supply_zones', [])
            if supply_zones:
                best_supply = max(supply_zones, key=lambda x: x['strength'])
                entry_price = best_supply['zone_low']
                stop_loss = best_supply['zone_high'] * 1.002
                take_profit = entry_price * 0.94  # 6% target
                entry_type = 'supply_zone_entry'
                logic = f"Entry at supply zone ${entry_price:.4f} dengan SL di atas zone"
            else:
                entry_price = current_price * 1.001
                stop_loss = current_price * 1.04
                take_profit = current_price * 0.94
                entry_type = 'market_entry'
                logic = "Market entry dengan supply bias"
            ...
            entry_type = 'no_entry'
            logic = "Tunggu sinyal yang lebih jelas"
        ...
        if confidence == 'High' and signal_strength == 'STRONG':
            entry_timing = 'Immediate'
        elif confidence in ['High', 'Medium']:
            entry_timing = 'Wait for confirmation'
        else:
            entry_timing = 'Wait for clearer signal'
    ...
    def _determine_confidence_level(self, snd_score, volume_analysis, order_flow):
        """Determine overall confidence level for the analysis"""
        confidence_factors = []
        
        # S&D score confidence
        score = snd_score['score']
        if score >= 75 or score <= 25:
            confidence_factors.append('extreme_snd_score')
        elif score >= 65 or score <= 35:
            confidence_factors.append('strong_snd_score')
        
        # Volume confidence
        volume_strength = volume_analysis['pressure_strength']
        if volume_strength >= 80:
            confidence_factors.append('strong_volume_confirmation')
        elif volume_strength >= 60:
            confidence_factors.append('moderate_volume_confirmation')
        
        # Order flow confidence
        order_strength = order_flow['imbalance_strength']
        if order_strength >= 80:
            confidence_factors.append('extreme_order_imbalance')
        elif order_strength >= 60:
            confidence_factors.append('strong_order_imbalance')
        
        # Determine overall confidence
        high_confidence_factors = ['extreme_snd_score', 'strong_volume_confirmation', 'extreme_order_imbalance']
        medium_confidence_factors = ['strong_snd_score', 'moderate_volume_confirmation', 'strong_order_imbalance']
        
        high_count = sum(1 for factor in confidence_factors if factor in high_confidence_factors)
        medium_count = sum(1 for factor in confidence_factors if factor in medium_confidence_factors)
    ...
    def _assess_snd_risks(self, snd_score, zones, current_price):
        """Assess risks based on S&D analysis"""
        risks = []
        risk_level = 'Medium'
        
        # Score-based risk assessment
        score = snd_score['score']
        if 45 <= score <= 55:  # Neutral zone
            risks.append("Score S&D neutral - sinyal tidak jelas")
            risk_level = 'High'
        
        # Zone proximity risks
        zone_context = zones['current_zone_context']
        if zone_context == 'near_demand':
            risks.append("Dekat demand zone - risk jika zone gagal hold")
        elif zone_context == 'near_supply':
            risks.append("Dekat supply zone - risk jika zone gagal sebagai resistance")
        elif zone_context == 'neutral':
            risks.append("Tidak ada zone signifikan - risk management extra diperlukan")
        
        # Confidence vs recommendation mismatch
        confidence = snd_score['confidence']
        recommendation = snd_score['recommendation']
        if confidence == 'Low' and recommendation in ['STRONG BUY', 'STRONG SELL']:
            risks.append("Confidence rendah untuk strong signal - false signal risk tinggi")
            risk_level = 'Very High'
        
        # General S&D risks
        risks.append("S&D analysis memerlukan konfirmasi price action")
        risks.append("Volume spike bisa invalidate zones")
        
        return {
            'risk_level': risk_level,
            'risk_factors': risks,
            'recommended_position_size': self._get_position_size_recommendation(risk_level, confidence)
        }

    def _get_position_size_recommendation(self, risk_level, confidence):
        """Get position size recommendation based on risk and confidence"""
    ...
    def generate_auto_snd_signal_summary(self, snd_analysis, language='id'):
        """Generate summary for automatic S&D signals"""
        try:
            if 'error' in snd_analysis:
                return f"❌ Error in S&D analysis: {snd_analysis['error']}"
            
            symbol = snd_analysis['symbol']
            current_price = snd_analysis['current_price']
            snd_score = snd_analysis['snd_score']
            signal_rec = snd_analysis['signal_recommendation']
            confidence = snd_analysis['confidence_level']
            risk_assessment = snd_analysis['risk_assessment']
            
            primary_rec = signal_rec['primary_recommendation']
            
            if language == 'id':
                summary = f"""🎯 **Auto S&D Signal: {symbol}**

💰 **Harga**: ${current_price:,.4f}
📊 **S&D Score**: {snd_score['score']}/100 ({snd_score['bias']})
🎯 **Signal**: {primary_rec['direction']} ({primary_rec['signal_strength']})
📈 **Confidence**: {confidence}

🚀 **Entry Setup**:
• Entry: ${primary_rec['entry_price']:,.4f}
• Stop Loss: ${primary_rec['stop_loss']:,.4f}
• Take Profit: ${primary_rec['take_profit']:,.4f}
• R/R: {primary_rec['risk_reward']:.1f}:1

💡 **Logic**: {primary_rec['logic']}
⏰ **Timing**: {signal_rec['entry_timing']}
📊 **Position Size**: {risk_assessment['recommended_position_size']} of portfolio
...
⚠️ **Risk Level**: {risk_assessment['risk_level']}"""
            ...
            return f"❌ Error generating S&D signal summary: {str(e)}"
