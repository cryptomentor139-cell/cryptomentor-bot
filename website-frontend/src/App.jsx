...
export default function App() {
  ...
  const fetchVerStatus = async () => {
    ...
    // Demo mode bypass — skip backend verification
    if (window._demoMode || localStorage.getItem('cm_token')?.startsWith('demo.')) {
      setVerStatus({ status: 'approved' });
      setVerLoading(false);
      return;
    }
    try {
    ...
    return (
      <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-4 text-slate-200 font-sans relative overflow-hidden selection:bg-fuchsia-500/30">
        ...
        <div className="max-w-md w-full bg-[#0a0a0a]/60 backdrop-blur-3xl border border-white/10 rounded-[2rem] md:rounded-[2.5rem] shadow-[0_0_50px_rgba(0,0,0,0.8)] p-8 md:p-10 text-center relative z-10 group hover:border-white/20 transition-all duration-500">
          ...
          <div id="tg-widget-root" className="flex justify-center min-h-[50px] items-center" />

          {/* ── Demo Login (localhost / testing only) ── */}
              ...
              <button
                onClick={() => {
                  const demoUser = {
                    ...
                    username: 'demouser',
                    photo_url: 'https://ui-avatars.com/api/?name=Demo+User&background=d946ef&color=fff&bold=true',
                    is_premium: true,
                    credits: 500,
                  };
                  const demoToken = 'demo.' + btoa(JSON.stringify({ sub: '999999999', first_name: 'Demo User', username: 'demouser', exp: 9999999999 })) + '.demo';
                  localStorage.setItem('cm_token', demoToken);
                  localStorage.setItem('cm_user', JSON.stringify(demoUser));
                  window._demoMode = true;
                  window.location.reload();
                }}
                className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-white/5 border border-white/10 text-slate-300 text-sm font-bold hover:bg-white/10 hover:border-white/20 transition-all"
              >
                <Bot size={16} className="text-fuchsia-400" />
                Preview as Demo User
              </button>
              <p className="text-slate-600 text-[10px] mt-2 text-center">Localhost only — not available in production</p>
            </div>
          )}
        </div>
      </div>
