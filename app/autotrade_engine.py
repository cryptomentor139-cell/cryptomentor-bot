...
FLIP_COOLDOWN_SECONDS = 1800  # 30 menit antar flip per simbol
FLIP_MIN_CONFIDENCE   = 75    # flip hanya jika sinyal baru sangat kuat


def _is_reversal(open_side: str, new_signal: Dict) -> bool:
    """
    Cek apakah sinyal baru adalah reversal dari posisi aktif.
    Syarat ketat ala professional trader:
    - Arah berlawanan
    - Confidence >= FLIP_MIN_CONFIDENCE
    - 1H trend sudah flip (bukan sekadar noise 15M)
    - Market structure berubah (CHoCH: Change of Character)
    """
    import time
    ...
    # Confidence harus tinggi
    conf = new_signal.get("confidence", 0)
    if conf < FLIP_MIN_CONFIDENCE:
        logger.info(f"[Reversal:{symbol}] No flip — confidence {conf}% < {FLIP_MIN_CONFIDENCE}%")
        return False

    # 1H trend harus sudah flip (bukan neutral)
    ...
    # Market structure harus konfirmasi (CHoCH)
    ...
    if elapsed < FLIP_COOLDOWN_SECONDS:
        logger.info(f"[Reversal:{symbol}] No flip — cooldown {int(FLIP_COOLDOWN_SECONDS - elapsed)}s remaining")
        return False

    logger.warning(f"[Reversal:{symbol}] ✅ ALL CONDITIONS MET — {open_side} → {new_side} | conf={conf}% | 1H={trend_1h} | struct={struct}")
    return True
...
def _compute_signal_pro(base_symbol: str, btc_bias: Optional[Dict] = None) -> Optional[Dict]:
    ...
    if base_symbol.upper() != "BTC" and btc_bias:
        ...
        # Require strong BTC bias for altcoin trades
        if btc_bias_dir == "NEUTRAL" or btc_strength < 60:
            logger.info(
                f"[Signal] {symbol} SKIPPED — BTC bias {btc_bias_dir} "
                f"strength={btc_strength}% (need ≥60% for altcoin)"
            )
            return None
        ...
        try:
            ...
            if open_positions:
                for pos in open_positions:
                    ...
                    if not rev_sig or not _is_reversal(pos_side, rev_sig):
                        continue
            ...
            await bot.send_message(
                ...
                text=(
                    ...
                    f"🧠 Reasons:\n{reasons_text}\n\n"
                    f"💰 Potential profit: +{reward_usdt:.2f} USDT\n"
                    f"⚠️ Potential loss: -{risk_usdt:.2f} USDT\n"
