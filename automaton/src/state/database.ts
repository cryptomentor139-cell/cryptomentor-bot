...
export function createDatabase(dbPath: string): AutomatonDatabase {
  ...
  const insertPaymentRequest = (request: PaymentRequest): void => {
    db.prepare(
      `INSERT INTO payment_requests (id, to_address, amount_cents, note, status, requested_at, reviewed_at, reviewed_by, rejection_reason, execution_result, executed_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      request.id,
      request.toAddress,
  ...
  const updatePaymentRequest = (
    ...
    values.push(id);
    const query = `UPDATE payment_requests SET ${fields.join(", ")} WHERE id = ?`;
    db.prepare(query).run(...values);
  };

  const getPaymentRequestById = (id: string): PaymentRequest | undefined => {
    const row = db
  ...
  const getPaymentRequestsByStatus = (status: PaymentStatus): PaymentRequest[] => {
    const rows = db
      .prepare("SELECT * FROM payment_requests WHERE status = ? ORDER BY requested_at DESC")
      .all(status) as any[];
    return rows.map(deserializePaymentRequest);
  };

  const getPaymentRequestsSince = (timestamp: string): PaymentRequest[] => {
    const rows = db
      .prepare("SELECT * FROM payment_requests WHERE requested_at >= ? ORDER BY requested_at DESC")
  ...
  const getTelegramConfig = (key: string): string | undefined => {
    const row = db
      .prepare("SELECT value FROM telegram_config WHERE key = ?")
      .get(key) as { value: string } | undefined;
    return row?.value;
  };

  const setTelegramConfig = (key: string, value: string): void => {
    db.prepare(
