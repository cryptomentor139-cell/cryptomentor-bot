...
/**
 * Determine the survival tier based on current credits and USDC balance.
 * USDC can be used for x402 payments, so we consider it as available funds.
 */
export function getSurvivalTier(creditsCents: number, usdcBalance: number = 0): SurvivalTier {
  // Convert USDC to cents for comparison (1 USDC = 100 cents)
  const usdcCents = Math.floor(usdcBalance * 100);
  const totalFundsCents = creditsCents + usdcCents;
  
  if (totalFundsCents > SURVIVAL_THRESHOLDS.normal) return "normal";
  if (totalFundsCents > SURVIVAL_THRESHOLDS.low_compute)
    return "low_compute";
  if (totalFundsCents > SURVIVAL_THRESHOLDS.dead) return "critical";
  return "dead";
}
