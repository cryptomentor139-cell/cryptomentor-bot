...
export function createConwayClient(
  ...
  // Use local sandbox if sandboxId is empty
  const useLocalSandbox = !sandboxId || sandboxId === "";
  const localSandbox = useLocalSandbox ? new LocalSandbox() : null;
  
  if (useLocalSandbox) {
    console.log(`[LOCAL MODE] Using local filesystem sandbox at: ${localSandbox!.getWorkDir()}`);
  }
  ...
  const exec = async (
    ...
    // Use local sandbox if available
    if (useLocalSandbox && localSandbox) {
      return await localSandbox.exec(command, timeout);
    }
  ...
  const writeFile = async (
    ...
    // Use local sandbox if available
    if (useLocalSandbox && localSandbox) {
      await localSandbox.writeFile(path, content);
      return;
    }
  ...
  const readFile = async (filePath: string): Promise<string> => {
    // Use local sandbox if available
    if (useLocalSandbox && localSandbox) {
      return await localSandbox.readFile(filePath);
    }
  ...
  const exposePort = async (port: number): Promise<PortInfo> => {
    // Use local sandbox if available
    if (useLocalSandbox && localSandbox) {
      const info = await localSandbox.exposePort(port);
      return {
        ...info,
        sandboxId: "local",
      };
  ...
  const removePort = async (port: number): Promise<void> => {
    // Use local sandbox if available
    if (useLocalSandbox && localSandbox) {
      await localSandbox.removePort(port);
      return;
    }
