...
# WL#1: baca dari config lokal, bukan env global
try:
    import config as wl_config
    SUPABASE_URL = (wl_config.SUPABASE_URL or "").rstrip("/")
    SUPABASE_SERVICE_KEY = wl_config.SUPABASE_SERVICE_KEY
except ImportError:
...
def _client() -> Client:
    if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
        raise RuntimeError("Set SUPABASE_URL & SUPABASE_SERVICE_KEY in .env (WL#1 instance).")
